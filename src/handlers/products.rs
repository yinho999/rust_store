...
// Add pool handlers
pub fn pool_handler(pool: web::Data<PgPool>) -> Result<PgPooledConnection, ServerError> {
    pool.get()
...
// Get a product by id
#[get("/{id}")]
pub async fn get(id: web::Path<i32>, pool: web::Data<PgPool>) -> Result<HttpResponse, ServerError> {
    let pool = pool_handler(pool)?;
